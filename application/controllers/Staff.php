...
class Staff extends CI_Controller {
    ...
    public function insert()
    {
        ...
        if($this->form_validation->run() !== false)
        {
            ...
            if($login>0)
            {
                $data=$this->Staff_model->insert_staff(array('EmployeeID'=>$empID, 'id'=>$login,'staff_name'=>$name,'gender'=>$gender,'email'=>$email,'password'=>$password,'mobile'=>$mobile,'dob'=>$dob,'doj'=>$doj,'address'=>$address,'city'=>$city,'state'=>$state,'country'=>$country,'department_id'=>$department,'pic'=>$image,'added_by'=>$added));
            }
    ...
    function edits($id)
    {
        ...
        $data['country']=$this->Home_model->select_countries();
        $data['content']=$this->Staff_model->select_staff_byID($id);
        $data['certificate']=$this->Staff_model->select_staff_certificate($id);
        $data['credential']=$this->Staff_model->select_staff_credential($id);
        $this->load->view('admin/header');
        $this->load->view('admin/Employee-info',$data);
        $this->load->view('admin/footer');
    }
